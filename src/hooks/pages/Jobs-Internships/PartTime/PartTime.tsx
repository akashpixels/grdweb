...
import { getPartTimeJobList } from "store/Slices/api.slice";
...
const PartTime = () => {
  ...
  const partTimeList = useAppSelector((state) => state.api.partTimeJobList);
  ...
  useEffect(() => {
    setLoading(true);
    if (!searchDataValue) {
      fetchFilterJobList("Part Time", pageIndex, DEFAULT_PAGE_SIZE).then((res) => {
        dispatch(getPartTimeJobList(res));
        setLoading(false);
      });
    } else {
      const searchData = {
  ...
  const handleSearch = (value?: SelectedFilters, pageNumber?: number) => {
    ...
    filterJobListing(filterData).then((results) => {
      dispatch(getPartTimeJobList(results));
      setLoading(false);
    });
  };
  ...
  const pageCount = Math.ceil(partTimeList?.totalRecord / ITEMS_PER_PAGE);
    ...
    <div className="flex-1 overflow-y-scroll custom-scroll mb-2">
      <div className="grid grid-cols-12 gap-6 h-full">
        <div className="col-span-12 h-full overflow-y-scroll custom-scroll">
          {!loading && partTimeList?.data ? (
            <PostContainer
              handleChange={handleSearchChange}
              openModal={openModal}
              jobList={partTimeList?.data}
              pageIndexChange={getPageIndexChange}
              totalCount={partTimeList?.totalRecord}
