...
const Sponsored = () => {
  ...
  const sponsoredJobList = useAppSelector((state) => state.api.sponsoredJobList);
  ...
  useEffect(() => {
    ...
    setLoading(true);
    if (!searchDataValue) {
      fetchFilterJobList("Sponsored", pageIndex, DEFAULT_PAGE_SIZE).then((res) => {
        dispatch(getSponsoredJobList(res));
        setLoading(false);
      });
    } else {
      const searchData = {
  ...
  const pageCount = Math.ceil(sponsoredJobList?.totalRecord / ITEMS_PER_PAGE);
  return (
    <div className="flex-1 overflow-y-scroll custom-scroll mb-2">
      <div className="grid grid-cols-12 gap-6 h-full">
        <div className="col-span-12 h-full overflow-y-scroll custom-scroll">
          {!loading && sponsoredJobList?.data ? (
            <PostContainer
              handleChange={(value: string) => handleSearchChange(value)}
              openModal={openModal}
              jobList={sponsoredJobList?.data}
              totalCount={sponsoredJobList?.totalRecord}
