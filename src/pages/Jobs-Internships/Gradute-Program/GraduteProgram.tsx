...
import { getGraduateProgramJobList, getInternJobList } from "store/Slices/api.slice";
...
const GraduateProgram = () => {
  ...
  const graduateProgram = useAppSelector((state) => state.api.graduateProgram);
  ...
  useEffect(() => {
    ...
    if (!searchDataValue) {
      fetchFilterJobList("graduateProgram", pageIndex, DEFAULT_PAGE_SIZE).then((res) => {
        dispatch(getGraduateProgramJobList(res));
        setLoading(false);
      });
    } else {
      const searchData = {
  ...
  const pageCount = Math.ceil(graduateProgram?.totalRecord / ITEMS_PER_PAGE);
    ...
    <div className="flex-1 overflow-y-scroll custom-scroll mb-2">
      <div className="grid grid-cols-12 gap-6 h-full">
        <div className="col-span-12 h-full overflow-y-scroll custom-scroll">
          {!loading && graduateProgram?.data ? (
            <PostContainer
              handleChange={handleSearchChange}
              openModal={openModal}
              jobList={graduateProgram?.data}
              pageIndexChange={getPageIndexChange}
              totalCount={graduateProgram?.totalRecord}
